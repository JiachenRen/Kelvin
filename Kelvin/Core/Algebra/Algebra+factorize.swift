...
public extension Algebra {
    ...
    private static func factorize(
        ...
        func combinedForms(of n: [Node], num: Int) -> [[Node]] {
            var cb = [[Node]]()
            let cs = Probability.combinations(of: (0..<n.count).map {$0}, num)
            for c in cs {
                var j = [Node]()
                var k = [Node]()
                var n = n
    ...
    /// Deconstructs a node into its arguments if it is `*`
    /// For nodes other than `*`, the node itself is returned
