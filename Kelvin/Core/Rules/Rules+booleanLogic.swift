    ...
    static let booleanLogic: [Operation] = [
        ...
        .unary(.not, [.function]) {
            ...
            case .or:
                ...
                return Function(.and, fun.map {!!$0})
            case .and:
                // not (a and b) is (not a or not b)
                return Function(.or, fun.map {!!$0})
            case .not:
                // not (not a) is a
                return fun[0]
            default:
        ...
        .binary(.and, [.node, .function]) {
            ...
            case .not:
                ...
                if let arg = fun.elements.first, arg === node {
                    return false
                }
            default:
                break
        ...
        .binary(.or, [.node, .function]) {
            ...
            case .and:
                ...
                let trial = try pack(.and, fun.map {
                    try (node ||| $0).simplify()
                })
                ...
                for (i, n) in fun.elements.enumerated() {
                    if let nestedOr = n as? Function, nestedOr.name == .or {
                        ...
                        var simplified = false
                        let trial = try pack(.or, nestedOr.map {
                            let org = (and &&& $0) ||| node
                            let sim = try org.simplify()
                            if sim.complexity < org.complexity {
                                simplified = true
        ...
        .binary(.and, Function.self, Function.self) {
            ...
            if fun1.name == fun2.name && fun1.name == .or {
                ...
                var leftArgs = fun1.elements
                var rightArgs = fun2.elements
